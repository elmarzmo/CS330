...
void SceneManager::LoadSceneTextures() {
	bool bReturn = false;
	bReturn = CreateGLTexture("textures/pavers.jpg", "floor");
	bReturn = CreateGLTexture("textures/red.jpg", "taperedCylinde");
	bReturn = CreateGLTexture("textures/gold.jpg", "torus");
	bReturn = CreateGLTexture("textures/brown.jpg", "stalk");
	bReturn = CreateGLTexture("textures/green.jpg", "leaf");
	bReturn = CreateGLTexture("textures/awesomeface.png", "awesome");
...
void SceneManager::DefineObjectMaterials() {
	// Define a material for a cement-like object
	...
	// Define a material for a gold-like object

	OBJECT_MATERIAL goldMaterial; 
	goldMaterial.ambientColor = glm::vec3(0.2f, 0.2f, 0.2f);
	goldMaterial.ambientStrength = 0.4f;
	goldMaterial.diffuseColor = glm::vec3(0.3f, 0.3f, 0.5f);
	goldMaterial.specularColor = glm::vec3(0.6f, 0.5f, 0.4f);
	goldMaterial.shininess = 22.0; goldMaterial.tag = "gold";
	m_objectMaterials.push_back(goldMaterial);
	// Define a material for a glass-like object
	...
	glassMaterial.shininess = 85.0; glassMaterial.tag = "glass";
	m_objectMaterials.push_back(glassMaterial);


}
// Function to set up the scene's lighting
void SceneManager::SetupSceneLights() {
	// Setup the first light source (Key Light)
	...
	// Setup the second light source (Fill Light)

	m_pShaderManager->setVec3Value("lightSources[1].position", 1.2f, 1.0f, 2.0f);
	...
	// Setup the third light source (Back Light)

	m_pShaderManager->setVec3Value("lightSources[2].position", 15.0f, 12.0f, -10.0f);
...
void SceneManager::RenderScene()
{
	...
	SetShaderTexture("awesome");
	SetShaderMaterial("gold");
	// draw the mesh with transformation values
	m_basicMeshes->DrawPlaneMesh();

	//creat the Vase with Leaves
	// 
	//TaperedCylinder
	...
	// set the XYZ position for the mesh
	positionXYZ = glm::vec3(5.0f, 2.0f, 3.0f);
	...
	//set shader texture
	SetShaderTexture("taperedCylinde");
	...
	// set the XYZ position for the mesh
	glm:: vec3 basePosition2 = glm::vec3(5.0f, 3.0f, 3.0f);
	 float verticalOffset2 = 1.5f; // Adjust this value to space out the toruses
	 
	 //blue color
	 SetShaderTexture("torus");
	 SetShaderMaterial("gold");

	// Draw three toruses stacked on top of each other using a loop
	for (int i = 0; i < 3; ++i) {
		// Calculate the position for each torus
		glm::vec3 positionXYZ = basePosition2 + glm::vec3(0.0f, i * verticalOffset2, 0.0f);
	...
	//first leaf stalk 
	scaleXYZ = glm::vec3(0.05f, 0.0f, 2.0f);

	// set the XYZ rotation for the mesh
	// rotate so it look like a leaf stalk 
	XrotationDegrees = 80.0f;
	YrotationDegrees = 0.0f;
	ZrotationDegrees = 0.0f;

	// set the XYZ position for the mesh
	positionXYZ = glm::vec3(5.0f, 8.0f, 2.5f);
	...
	// set the XYZ position for the mesh
	positionXYZ = glm::vec3(4.8f, 8.0f, 2.5f);
	...
	//first leaf  
	scaleXYZ = glm::vec3(1.0f, 0.0f, 1.0f);
	...
	// set the XYZ position for the mesh
	positionXYZ = glm::vec3(5.0f, 9.7f, 2.0f);
	...
	// set the XYZ position for the mesh
	positionXYZ = glm::vec3(4.8f, 9.7f, 2.0f);
